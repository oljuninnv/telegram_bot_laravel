...
Дополнением к боту является рабочая админ-панель, в которой реализован функционал бота по формированию отчётов и настройки периодов сбора.

Проект реализован на связке технологий **Laravel**, **Telegram Bot SDK** и панели администратора**MoonShine**, что обеспечивает гибкость и масштабируемость решения.
...
7. Запустите seeder командой, который заполнит ваши таблици настройками и хэштегами по умолчанию:
  ...
  php artisan db:seed
  ```
8. Создайте нового супер пользователя для работы с админ-панелью moonshine:
  ```
...
- WEBHOOK_URL="{ссылка на ваш webhok}"
- TELEGRAM_USER_ADMIN_ID="ваш id в телеграмм"
- TELEGRAM_BOT_URL="https://{ссылка на вашего бота}"
...
2. Добавьте задачу с запуском PHP, аргументом artisan schedule:run (в случае если вы используете Windows Task Scheduler);
3. Установите триггер на ежедневное выполнение с интервалом в час.
